...
class Watchlist:
    ...
    def add_to_watchlist(self,symbol,notes="added!"):
        ...
        if price is not None:
            self.watchlist[symbol] = {'added_on': date.today().strftime('%Y-%m-%d'), 'last_price': price, 'notes':notes}
...
def create_database():
    ...
    # Portfolios Table (stores serialized portfolio objects)
...
def save_portfolio(user_id, portfolio_obj):
    """Improved portfolio save function with create/update detection"""
    conn = sqlite3.connect('trading_system.db')
    try:
        # Serialize portfolio data with current timestamp
        portfolio_data = {
            'name': portfolio_obj.name,
            'cash': portfolio_obj.portfolio['cash'],
            'holdings': portfolio_obj.portfolio['holdings'],
            'transactions': portfolio_obj.portfolio['transactions'],
            'logs': portfolio_obj.logs,
            'timestamp': datetime.now().isoformat()
        }
        json_data = json.dumps(portfolio_data)

        # Check if portfolio has database ID (exists in DB)
        if hasattr(portfolio_obj, 'db_id'):
            # Update existing portfolio
            conn.execute('''UPDATE portfolios 
                            SET name = ?, data = ?
                            WHERE id = ? AND user_id = ?''',
                         (portfolio_obj.name, json_data,
                          portfolio_obj.db_id, user_id))
            action = "updated"
        else:
            # Create new portfolio entry
            cursor = conn.cursor()
            cursor.execute('''INSERT INTO portfolios (user_id, name, data)
                            VALUES (?, ?, ?)''',
                           (user_id, portfolio_obj.name, json_data))
            # Set the database ID on the portfolio object
            portfolio_obj.db_id = cursor.lastrowid
            action = "created"

        conn.commit()
        print(f"Portfolio successfully {action} [ID: {getattr(portfolio_obj, 'db_id', 'new')}]")
        return True
    except Exception as e:
...
def load_portfolio(user_id, portfolio_id):
    """Enhanced portfolio loading with database ID tracking"""
    conn = sqlite3.connect('trading_system.db')
    try:
        c = conn.cursor()
        ...
        # Unpack results with database ID
        db_id, name, data_str = result
        data = json.loads(data_str)

        # Reconstruct portfolio object
        portfolio = Simulation(name, data['cash'])
        portfolio.db_id = db_id  # Track database ID

        # Restore state
        portfolio.portfolio['holdings'] = data['holdings']
        portfolio.portfolio['transactions'] = data['transactions']
        portfolio.logs = data['logs']

        print(f"Loaded portfolio '{name}' (ID: {db_id})")
        return portfolio
    except Exception as e:
